...
def resolve_environment(
    ...
    # plumi-native selection outranks envo: a runner that never sees envo
    # still names its environment honestly
    plumi_environment: str | None = environ.get("PLUMI_ENVIRONMENT")
    if plumi_environment and plumi_environment != "localhost":
        return Environment(
            name=plumi_environment,
            organization=organization,
            domain=domain,
            region=read_profile_option(plumi_environment, "region", aws_config_path),
        )
