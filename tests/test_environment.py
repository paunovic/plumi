...
def test_plumi_environment_outranks_envo(tmp_path):
    aws_config = tmp_path / "config"
    aws_config.write_text("[profile prod]\nregion = us-east-1\n")

    environment = resolve_environment(
        environ={"PLUMI_ENVIRONMENT": "prod", "ENVO_ENVIRONMENT": "qa"},
        aws_config_path=aws_config,
    )
    ...
    assert environment.region == "us-east-1"
    assert environment.state_bucket == "pulumi-state-prod-acme-com"


def test_plumi_environment_without_a_profile_carries_no_region(tmp_path):
    environment = resolve_environment(
        environ={"PLUMI_ENVIRONMENT": "qa"},
